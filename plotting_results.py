...
surface_plot_vec(x_plt.flatten(), y_plt.flatten(), T_pinn.flatten(), "Regressed temperature field", "T_regressed")

# plot the regressed thermal conductivity
h_pinn = condNet(inputs).data.cpu().numpy()
surface_plot_vec(x_plt.flatten(), y_plt.flatten(), h_pinn.flatten(), "Regressed thermal conductivity", "h_regressed")

# plot the actual temperature field
surface_plot_vec(data[:,1], data[:,2], data[:,0], "FEM temperature field", "T_exact")

# plot the actual thermal conductivity
h_plt = h_exact(x_plt, y_plt)
surface_plot_vec(x_plt.flatten(), y_plt.flatten(), h_plt.flatten(), "Exact thermal conductivity", "h_exact")
